...
export default function page({ params }) {
    ...
    function getAdvertiseHere() {
        if (adminData) {
            axios({
                url: `${apiData}/admin/edit/advertiseHere/${params.id}`,
                method: 'get',
                headers: {
            ...
            return (
                <div className="flex flex-col gap-6">
                    ...
                    <div className="p-2 border-r-red-700 border-r-8 border">
                        رقم الهاتف : {data.AdvertiseHere?.phone}
                    </div>
                    <div className="p-2 border-r-red-700 border-r-8 border min-h-[100px]">
                        الرساله : {data.AdvertiseHere?.message}
                    </div>
                    <div className=" flex justify-center gap-6">
                        <Link href={`tel:${data.AdvertiseHere?.phone}`} className="flex gap-3 px-6 py-2 rounded-lg bg-gray-600 items-center text-white hover:bg-gray-500">
                            <span>اتصال</span>
    ...
    function Delete(){
        setLoader(true)
        axios({
            url: `${apiData}/admin/delete/advertiseHere/${params.id}`,
            method: 'delete',
            headers: {
                'Authorization': `Bearer ${adminData.access_token}`
            }
            ...
            router.replace("/dashboard/advertiseHere")
