...
export default function Contact() {
    ...
    function getData() {
        if (adminData) {
            axios({
                url: `${apiData}/admin/showDeleted/contactUs`,
                method: 'get',
                headers: {
                        ...
                        data.map((el, i) => {
                            ...
                            return (
                                <div key={i} className={`w-full grid grid-cols-4 ${bg} p-1`}>
                                    ...
                                    <p className="text-center truncate">{el.contact.message}</p>
                                    <p className="text-center ">{el.contact.phone}</p>
                                    <div className="text-center flex items-center justify-center gap-2">
                                        <button onClick={()=>{deleteData(el.contact.id)}} className="bg-red-700 text-white px-2 rounded-md">حذف</button>
                                        <button onClick={()=>{restData(el.contact.id)}} className="bg-sky-700 text-white px-2 rounded-md">استرجاع</button>
                                    </div>
                                </div>
                            )
                        })
    ...
    function deleteData(id){
        setLoader(true)
        axios({
            url: `${apiData}/admin/forceDelete/contactUs/${id}`,
            method: 'delete',
            headers: {
                'Authorization': `Bearer ${adminData.access_token}`
            }
    ...
    function restData(id){
        setLoader(true)
        axios({
            url: `${apiData}admin/restore/contactUs/${id}`,
            method: 'get',
            headers: {
                'Authorization': `Bearer ${adminData.access_token}`
            }
