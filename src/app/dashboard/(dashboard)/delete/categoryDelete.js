...
export default function CategoryDelete() {
    ...
    function getData() {
        if (adminData) {
            axios({
                url: `${apiData}/admin/showDeleted/category`,
                method: 'get',
                headers: {
                        ...
                        data.map((el, i) => {
                            ...
                            return (
                                <div key={i} className={`w-full grid grid-cols-4 ${bg} p-1`}>
                                    ...
                                    <p className="text-center truncate">{el.news_count}</p>
    ...
    function deleteData(id){
        setLoader(true)
        axios({
            url: `${apiData}/admin/forceDelete/category/${id}`,
            method: 'delete',
            headers: {
                'Authorization': `Bearer ${adminData.access_token}`
            }
    ...
    function restData(id){
        setLoader(true)
        axios({
            url: `${apiData}/admin/restore/category/${id}`,
            method: 'get',
            headers: {
                'Authorization': `Bearer ${adminData.access_token}`
            }
