...
export default function MessDelete() {
    ...
    function getData() {
        if (adminData) {
            axios({
                url: `${apiData}/admin/showDeleted/advertiseHere`,
                method: 'get',
                headers: {
                        ...
                        data.map((el, i) => {
                            ...
                            return (
                                <div key={i} className={`w-full grid grid-cols-4 ${bg} p-1`}>
                                    ...
                                    <p className="text-center truncate">{el.AdvertiseHere.message}</p>
                                    <p className="text-center ">{el.AdvertiseHere.phone}</p>
                                    <div className="text-center flex items-center justify-center gap-2">
                                        <button onClick={()=>{deleteData(el.AdvertiseHere.id)}} className="bg-red-700 text-white px-2 rounded-md">حذف</button>
                                        <button onClick={()=>{restData(el.AdvertiseHere.id)}} className="bg-sky-700 text-white px-2 rounded-md">استرجاع</button>
                                    </div>
                                </div>
                            )
                        })
    ...
    function deleteData(id){
        setLoader(true)
        axios({
            url: `${apiData}/admin/forceDelete/advertiseHere/${id}`,
            method: 'delete',
            headers: {
                'Authorization': `Bearer ${adminData.access_token}`
            }
    ...
    function restData(id){
        setLoader(true)
        axios({
            url: `${apiData}/admin/restore/advertiseHere/${id}`,
            method: 'get',
            headers: {
                'Authorization': `Bearer ${adminData.access_token}`
            }
    ...
    return (
        <div className="w-full overflow-auto">
            ...
            <div className="p-2 px-4 border flex justify-between bg-white border-r-8 border-r-red-700 items-center">
                <p className="font-bold">رسائل المعلنين</p>
            </div>
            {FunSetData()}
        </div>
    )
