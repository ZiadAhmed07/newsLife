...
export default function AdminDelete() {
    ...
    function getData() {
        if (adminData) {
            axios({
                url: `${apiData}/admin/showDeleted/admin`,
                method: 'get',
                headers: {
        ...
        if (data) {
            return (
                <div className="w-full h-[300px] bg-white border min-w-[500px]">
                    <div className="w-full grid grid-cols-4 p-1 font-bold">
                        <p className="text-center">الاسم</p>
                        <p className="text-center">البريد</p>
                        ...
                        data.map((el, i) => {
                            ...
                            return (
                                <div key={i} className={`w-full grid grid-cols-4 ${bg} p-1`}>
                                    ...
                                    <p className="text-center truncate">{el.email}</p>
                                    <p className="text-center">{el.role.name}</p>
    ...
    function deleteData(id){
        setLoader(true)
        axios({
            url: `${apiData}/admin/forceDelete/admin/${id}`,
            method: 'delete',
            headers: {
                'Authorization': `Bearer ${adminData.access_token}`
            }
    ...
    function restData(id){
        setLoader(true)
        axios({
            url: `${apiData}/admin/restore/admin/${id}`,
            method: 'get',
            headers: {
                'Authorization': `Bearer ${adminData.access_token}`
            }
