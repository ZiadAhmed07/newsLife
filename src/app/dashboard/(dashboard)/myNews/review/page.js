...
export default function page() {
    ...
    function getData() {
        if (adminData) {
            axios({
                url: `${apiData}/admin/edit/adminProfile/with/reviewed/news/${adminData.admin.id}?page=${countPage}`,
                method: 'get',
                headers: {
                ...
                <div className=" w-full grid gap-4 sm:grid-cols-2 lg:grid-cols-3 p-4">
                    {
                        data.news.map((e, index) => {
                            return (
                                <Link href={`/dashboard/myNews/${e.news_id}`} key={index} className='hover:opacity-80 transition-all flex flex-col gap-2 shadow-md relative'>
        ...
        <div className="w-full overflow-auto">
            <div className="p-2 px-4  border flex justify-between bg-white border-r-8 border-r-red-700">
                ...
                <div className=" flex text-sm text-gray-600 gap-4 items-center">
                    ...
                    <Link href={'/dashboard/myNews/review'} className="bg-red-700 rounded-md p-1 text-white">  قيد المراجعه</Link>
