...
export default function Sitbar() {
    ...
    return (
        <div className="w-[200px] h-screen bg-white fixed z-30 max-sm:hidden overflow-auto">
            {FunLoader()}
            {
                        ...
                        {Links1.map((el, index) => {
                            ...
                            return (
                                <li key={index}>
                                    <a
                                        href={el.Link}
                                        className={` cursor-pointer flex gap-4 text-sm hover:bg-gray-200 p-2 ${isActive ? 'border-r-4 border-r-red-700 bg-gray-200' : ''} ${hasPermission ? '' : 'hidden'}`}
                                    >
                                        {el.svg}
                                        <span>{el.lable}</span>
                                    </a>
